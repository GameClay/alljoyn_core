/**
 * @file
 * The simple name service implementation
 */
...
//
const char* SimpleNameService::IPV4_MULTICAST_GROUP = "239.255.37.41";
const uint16_t SimpleNameService::MULTICAST_PORT = 9956;
...
// translate it.
//
const char* SimpleNameService::IPV6_MULTICAST_GROUP = "ff03::efff:2529";
...
SimpleNameService::SimpleNameService()
    : Thread("SimpleNameService"), m_state(IMPL_SHUTDOWN), m_callback(0),
...
{
    QCC_DbgPrintf(("SimpleNameService::SimpleNameService()\n"));
}
...
{
    uint32_t ourFlags = 0;
    if (flags & IFF_UP) ourFlags |= SimpleNameService::IfConfigEntry::UP;
    if (flags & IFF_BROADCAST) ourFlags |= SimpleNameService::IfConfigEntry::BROADCAST;
    if (flags & IFF_DEBUG) ourFlags |= SimpleNameService::IfConfigEntry::DEBUG;
    if (flags & IFF_LOOPBACK) ourFlags |= SimpleNameService::IfConfigEntry::LOOPBACK;
    if (flags & IFF_POINTOPOINT) ourFlags |= SimpleNameService::IfConfigEntry::POINTOPOINT;
    if (flags & IFF_RUNNING) ourFlags |= SimpleNameService::IfConfigEntry::RUNNING;
    if (flags & IFF_NOARP) ourFlags |= SimpleNameService::IfConfigEntry::NOARP;
    if (flags & IFF_PROMISC) ourFlags |= SimpleNameService::IfConfigEntry::PROMISC;
    if (flags & IFF_NOTRAILERS) ourFlags |= SimpleNameService::IfConfigEntry::NOTRAILERS;
    if (flags & IFF_ALLMULTI) ourFlags |= SimpleNameService::IfConfigEntry::ALLMULTI;
    if (flags & IFF_MASTER) ourFlags |= SimpleNameService::IfConfigEntry::MASTER;
    if (flags & IFF_SLAVE) ourFlags |= SimpleNameService::IfConfigEntry::SLAVE;
    if (flags & IFF_MULTICAST) ourFlags |= SimpleNameService::IfConfigEntry::MULTICAST;
    if (flags & IFF_PORTSEL) ourFlags |= SimpleNameService::IfConfigEntry::PORTSEL;
    if (flags & IFF_AUTOMEDIA) ourFlags |= SimpleNameService::IfConfigEntry::AUTOMEDIA;
    if (flags & IFF_DYNAMIC) ourFlags |= SimpleNameService::IfConfigEntry::DYNAMIC;
    return ourFlags;
}


QStatus SimpleNameService::IfConfig(std::vector<IfConfigEntry>& entries)
{
    QCC_DbgPrintf(("SimpleNameService::IfConfig(): The Linux way\n"));
...
QStatus SimpleNameService::IfConfig(std::vector<IfConfigEntry>& entries)
{
    QCC_DbgPrintf(("SimpleNameService::IfConfig(): The Windows way\n"));
...
QStatus SimpleNameService::Init(const qcc::String& guid, bool enableIPv4, bool enableIPv6, bool loopback)
{
    QCC_DbgPrintf(("SimpleNameService::Init()\n"));
...
QStatus SimpleNameService::OpenInterface(const qcc::String& name)
{
    QCC_DbgPrintf(("SimpleNameService::OpenInterface(%s)\n", name.c_str()));

    //
    // Can only call OpenInterface() if the object is running.
    //
    if (m_state != IMPL_RUNNING) {
        QCC_DbgPrintf(("SimpleNameService::OpenInterface(): Not running\n"));
        return ER_FAIL;
    }
    ...
    for (uint32_t i = 0; i < m_requestedInterfaces.size(); ++i) {
        if (m_requestedInterfaces[i].m_interfaceName == name) {
            QCC_DbgPrintf(("SimpleNameService::OpenInterface(): Already opened.\n"));
            m_mutex.Unlock();
            return ER_OK;
        }
    }
...
QStatus SimpleNameService::OpenInterface(const qcc::IPAddress& addr)
{
    QCC_DbgPrintf(("SimpleNameService::OpenInterface(%s)\n", addr.ToString().c_str()));
    ...
    if (addr == qcc::IPAddress("0.0.0.0") ||
        ...
        addr == qcc::IPAddress("::")) {
        QCC_DbgPrintf(("SimpleNameService::OpenInterface(): Wildcard address\n"));
        m_any = true;
        m_mutex.Unlock();
        return ER_OK;
    }
...
QStatus SimpleNameService::CloseInterface(const qcc::String& name)
{
    QCC_DbgPrintf(("SimpleNameService::SloseInterface(%s)\n", name.c_str()));

    //
    // Can only call CloseInterface() if the object is running.
    //
    if (m_state != IMPL_RUNNING) {
        QCC_DbgPrintf(("SimpleNameService::CloseInterface(): Not running\n"));
        return ER_FAIL;
    }
...
QStatus SimpleNameService::CloseInterface(const qcc::IPAddress& addr)
{
    QCC_DbgPrintf(("SimpleNameService::CloseInterface(%s)\n", addr.ToString().c_str()));
    ...
    if (addr == qcc::IPAddress("0.0.0.0") ||
        addr == qcc::IPAddress("0::0") ||
        addr == qcc::IPAddress("::")) {
        QCC_DbgPrintf(("SimpleNameService::CloseInterface(): Wildcard address\n"));
        m_any = false;
        m_mutex.Unlock();
        return ER_OK;
    }
...
void SimpleNameService::ClearLiveInterfaces(void)
{
    QCC_DbgPrintf(("SimpleNameService::ClearLiveInterfaces()\n"));
    ...
    for (uint32_t i = 0; i < m_liveInterfaces.size(); ++i) {
        ...
        if (m_liveInterfaces[i].m_address.IsIPv4()) {
            ...
            if (setsockopt(m_liveInterfaces[i].m_sockFd, IPPROTO_IP, IP_DROP_MEMBERSHIP,
                ...
                QCC_DbgPrintf(("SimpleNameService::ClearLiveInterfaces(void): setsockopt(IP_DROP_MEMBERSHIP) failed\n"));
            }
            ...
            if (INET_PTON(AF_INET6, mcGroup.c_str(), &mreq.ipv6mr_multiaddr) == 0) {
                QCC_DbgPrintf(("SimpleNameService::ClearLiveInterfaces(void): INET_PTON failed\n"));
                continue;
            }
            ...
            if (getsockopt(m_liveInterfaces[i].m_sockFd, IPPROTO_IPV6, IPV6_MULTICAST_IF,
                ...
                QCC_DbgPrintf(("SimpleNameService::ClearLiveInterfaces(void): getsockopt(IPV6_MULTICAST_IF) failed\n"));
                continue;
            }
            ...
            if (setsockopt(m_liveInterfaces[i].m_sockFd, IPPROTO_IPV6, IPV6_DROP_MEMBERSHIP,
                           reinterpret_cast<const char*>(&mreq), sizeof(mreq)) < 0) {
                QCC_DbgPrintf(("SimpleNameService::ClearLiveInterfaces(void): setsockopt(IPV6_DROP_MEMBERSHIP) failed\n"));
                continue;
            }
        } else {
            QCC_LogError(ER_FAIL, ("SimpleNameService:ClearLiveInterfaces: Address not IPv4 or IPv6"));
        }
...
//
void SimpleNameService::LazyUpdateInterfaces(void)
{
    QCC_DbgPrintf(("SimpleNameService::LazyUpdateInterfaces()\n"));
    ...
    //
    QCC_DbgPrintf(("SimpleNameService::LazyUpdateInterfaces(): IfConfig()\n"));
    ...
    for (uint32_t i = 0; i < entries.size(); ++i) {
        ...
        if (m_any) {
            ...
            for (uint32_t j = 0; j < m_requestedInterfaces.size(); ++j) {
                ...
                if (m_requestedInterfaces[j].m_interfaceName.size() != 0 &&
                    ...
                    QCC_DbgPrintf(("SimpleNameService::LazyUpdateInterfaces(): Found matching requestedInterface name\n"));
                    useEntry = true;
                    break;
                }
                ...
                if (m_requestedInterfaces[j].m_interfaceName.size() == 0 &&
                    ...
                    QCC_DbgPrintf(("SimpleNameService::LazyUpdateInterfaces(): Found matching requestedInterface address\n"));
                    useEntry = true;
                    break;
                }
            }
        ...
        if (useEntry == false || (entries[i].m_family != AF_INET && entries[i].m_family != AF_INET6)) {
            QCC_DbgPrintf(("SimpleNameService::LazyUpdateInterfaces(): Won't use this IfConfig entry\n"));
            continue;
        }
        ...
        if (setsockopt(sockFd, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&yes), sizeof(yes)) < 0) {
            QCC_LogError(status, ("SimpleNameService::LazyUpdateInterfaces(): setsockopt(SO_REUSEADDR) failed: %d - %s",
        ...
        if (entries[i].m_family == AF_INET) {
            if (setsockopt(sockFd, IPPROTO_IP, IP_MULTICAST_TTL, reinterpret_cast<const char*>(&ttl), sizeof(ttl)) < 0) {
                QCC_LogError(status, (
                                 "SimpleNameService::LazyUpdateInterfaces(): setsockopt(IP_MULTICAST_TTL) failed: %d - %s",
                                 errno, strerror(errno)));
                qcc::Close(sockFd);
                continue;
            }
        ...
        if (entries[i].m_family == AF_INET6) {
            if (setsockopt(sockFd, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, reinterpret_cast<const char*>(&ttl), sizeof(ttl)) < 0) {
                QCC_LogError(status, (
                                 "SimpleNameService::LazyUpdateInterfaces(): setsockopt(IP_MULTICAST_HOPS) failed: %d - %s",
                                 errno, strerror(errno)));
                qcc::Close(sockFd);
                continue;
            }
        ...
        if (entries[i].m_family == AF_INET) {
            ...
            if (setsockopt(sockFd, IPPROTO_IP, IP_MULTICAST_IF,
                ...
                QCC_LogError(status, (
                                 "SimpleNameService::LazyUpdateInterfaces(): setsockopt(IP_MULTICAST_IF) failed: %d - %s",
                                 errno, strerror(errno)));
                qcc::Close(sockFd);
                continue;
            }
        ...
        if (entries[i].m_family == AF_INET6) {
            ...
            if (setsockopt(sockFd, IPPROTO_IPV6, IPV6_MULTICAST_IF,
                ...
                QCC_LogError(status, (
                                 "SimpleNameService::LazyUpdateInterfaces(): setsockopt(IPV6_MULTICAST_IF) failed: %d - %s",
                                 errno, strerror(errno)));
                qcc::Close(sockFd);
                continue;
            }
        ...
        if (entries[i].m_family == AF_INET) {
            ...
            if (status != ER_OK) {
                QCC_LogError(status, ("SimpleNameService::LazyUpdateInterfaces(): bind(0.0.0.0) failed"));
                qcc::Close(sockFd);
                continue;
            }
        }
        ...
        if (entries[i].m_family == AF_INET6) {
            ...
            if (status != ER_OK) {
                QCC_LogError(status, ("SimpleNameService::LazyUpdateInterfaces(): bind(::) failed"));
                qcc::Close(sockFd);
                continue;
            }
        }
        ...
        if (entries[i].m_family == AF_INET) {
            ...
            if (setsockopt(sockFd, IPPROTO_IP, IP_ADD_MEMBERSHIP, reinterpret_cast<const char*>(&mreq), sizeof(mreq)) < 0) {
                QCC_LogError(status, (
                                 "SimpleNameService::LazyUpdateInterfaces(): setsockopt(IP_ADD_MEMBERSHIP) failed: %d - %s",
                                 errno, strerror(errno)));
                qcc::Close(sockFd);
                continue;
            }
        ...
        if (entries[i].m_family == AF_INET6) {
            ...
            if (INET_PTON(AF_INET6, mcGroup.c_str(), &mreq.ipv6mr_multiaddr) == 0) {
                QCC_LogError(status, ("SimpleNameService::LazyUpdateInterfaces(): INET_PTON failed"));
                qcc::Close(sockFd);
                continue;
            }
            ...
            if (setsockopt(sockFd, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP,
                           reinterpret_cast<const char*>(&mreq), sizeof(mreq)) < 0) {
                QCC_LogError(status, (
                                 "SimpleNameService::LazyUpdateInterfaces(): setsockopt(IPV6_ADD_MEMBERSHIP) failed: %d-%s",
                                 errno, strerror(errno)));
                qcc::Close(sockFd);
                continue;
            }
...
QStatus SimpleNameService::Locate(const qcc::String& wkn, LocatePolicy policy)
{
    QCC_DbgPrintf(("SimpleNameService::Locate(): %s with policy %d\n", wkn.c_str(), policy));
...
void SimpleNameService::SetCriticalParameters(
    uint32_t tDuration,
...
void SimpleNameService::SetCallback(Callback<void, const qcc::String&, const qcc::String&, vector<qcc::String>&, uint8_t>* cb)
{
    QCC_DbgPrintf(("SimpleNameService::SetCallback()\n"));

    delete m_callback;
    m_callback = cb;
}

QStatus SimpleNameService::SetEndpoints(
    const qcc::String& ipv4address,
    const qcc::String& ipv6address,
    uint16_t port)
{
    QCC_DbgPrintf(("SimpleNameService::SetEndpoints(%s, %s, %d)\n", ipv4address.c_str(), ipv6address.c_str(), port));
    ...
    if (ipv4address.size()) {
        if (ipv4address == "0.0.0.0") {
            QCC_DbgPrintf(("SimpleNameService::SetEndpoints(): IPv4 address looks like INADDR_ANY\n"));
            return ER_FAIL;
        }

        if (WildcardMatch(ipv4address, "*255") == 0) {
            QCC_DbgPrintf(("SimpleNameService::SetEndpoints(): IPv4 address looks like a broadcast address\n"));
            return ER_FAIL;
        }

        if (WildcardMatch(ipv4address, "127*") == 0) {
            QCC_DbgPrintf(("SimpleNameService::SetEndpoints(): IPv4 address looks like a loopback address\n"));
            return ER_FAIL;
        }
    }
    ...
    if (ipv6address.size()) {
        if (ipv6address == "0:0:0:0:0:0:0:1") {
            QCC_DbgPrintf(("SimpleNameService::SetEndpoints(): IPv6 address looks like a loopback address\n"));
            return ER_FAIL;
        }
        ...
        if (ipv6address == "::") {
            QCC_DbgPrintf(("SimpleNameService::SetEndpoints(): IPv6 address looks like in6addr_any\n"));
            return ER_FAIL;
        }
        ...
        if (ipv6address == "ff*") {
            QCC_DbgPrintf(("SimpleNameService::SetEndpoints(): IPv6 address looks like a multicast address\n"));
            return ER_FAIL;
        }
    }
    ...
    //
    if (port == 0) {
        QCC_DbgPrintf(("SimpleNameService::SetEndpoints(): Must provide non-zero port\n"));
        return ER_FAIL;
    }
...
QStatus SimpleNameService::Advertise(const qcc::String& wkn)
{
    QCC_DbgPrintf(("SimpleNameService::Advertise(): %s\n", wkn.c_str()));
...
QStatus SimpleNameService::Advertise(vector<qcc::String>& wkn)
{
    QCC_DbgPrintf(("SimpleNameService::Advertise()\n"));

    if (m_state != IMPL_RUNNING) {
        QCC_DbgPrintf(("SimpleNameService::Advertise(): Not IMPL_RUNNING\n"));
        return ER_FAIL;
    }
    ...
    //
    if (m_port == 0) {
        QCC_DbgPrintf(("SimpleNameService::Advertise(): Port not set\n"));
        return ER_FAIL;
    }
    ...
    for (uint32_t i = 0; i < wkn.size(); ++i) {
        ...
        if (j == m_advertised.end()) {
            ...
            //
            QCC_DbgPrintf(("SimpleNameService::Advertise(): Duplicate advertisement\n"));
            m_mutex.Unlock();
            return ER_OK;
        }
    }
...
QStatus SimpleNameService::Cancel(const qcc::String& wkn)
{
    QCC_DbgPrintf(("SimpleNameService::Cancel(): %s\n", wkn.c_str()));
...
QStatus SimpleNameService::Cancel(vector<qcc::String>& wkn)
{
    QCC_DbgPrintf(("SimpleNameService::Cancel()\n"));
...
void SimpleNameService::QueueProtocolMessage(Header& header)
{
    QCC_DbgPrintf(("SimpleNameService::QueueProtocolMessage()\n"));
    m_mutex.Lock();
...
void SimpleNameService::SendProtocolMessage(qcc::SocketFd sockFd, bool sockFdIsIPv4, Header& header)
{
    QCC_DbgPrintf(("SimpleNameService::SendProtocolMessage()\n"));
...
#if HAPPY_WANDERER
    if (Wander() == false) {
        QCC_LogError(ER_FAIL, ("SimpleNameService::SendProtocolMessage(): Wander(): out of range"));
        return;
    } else {
        QCC_LogError(ER_FAIL, ("SimpleNameService::SendProtocolMessage(): Wander(): in range"));
    }
#endif
...
void* SimpleNameService::Run(void* arg)
{
    QCC_DbgPrintf(("SimpleNameService::Run()\n"));
    ...
    while (!IsStopping()) {
        ...
        if (status != ER_OK && status != ER_TIMEOUT) {
            QCC_LogError(status, ("SimpleNameService::Run(): Event::Wait(): Failed"));
            break;
        }
        ...
        for (vector<qcc::Event*>::iterator i = signaledEvents.begin(); i != signaledEvents.end(); ++i) {
            if (*i == &stopEvent) {
                QCC_DbgPrintf(("SimpleNameService::Run(): Stop event fired\n"));
                //
                ...
                QCC_DbgPrintf(("SimpleNameService::Run(): Timer event fired\n"));
                //
                ...
                QCC_DbgPrintf(("SimpleNameService::Run(): Wake event fired\n"));
                //
                ...
                QCC_DbgPrintf(("SimpleNameService::Run(): Socket event fired\n"));
                //
                // This must be activity on one of our multicast listener sockets.
                //
                qcc::SocketFd sockFd = (*i)->GetFD();

                QCC_DbgPrintf(("SimpleNameService::Run(): Call qcc::RecvFrom()\n"));
                ...
                if (status != ER_OK) {
                    ...
                    //
                    QCC_LogError(status, ("SimpleNameService::Run(): qcc::RecvFrom(): Failed"));
                    break;
                }
...
void SimpleNameService::Retry(void)
{
...
void SimpleNameService::Retransmit(void)
{
    QCC_DbgPrintf(("SimpleNameService::Retransmit()\n"));

    //
    // We need a valid port before we send something out to the local subnet.
    //
    if (m_port == 0) {
        QCC_DbgPrintf(("SimpleNameService::Retransmit(): Port not set\n"));
        return;
    }
...
void SimpleNameService::DoPeriodicMaintenance(void)
{
#if HAPPY_WANDERER
    Wander();
#endif
    ...
    if (m_timer) {
        ...
        if (m_timer == m_tRetransmit) {
            QCC_DbgPrintf(("SimpleNameService::DoPeriodicMaintenance(): Retransmit()\n"));
            Retransmit();
            m_timer = m_tDuration;
        }
    }
...
void SimpleNameService::HandleProtocolQuestion(WhoHas whoHas, qcc::IPAddress address)
{
    QCC_DbgPrintf(("SimpleNameService::HandleProtocolQuestion()\n"));
    ...
    for (uint32_t i = 0; i < whoHas.GetNumberNames(); ++i) {
        ...
        for (list<qcc::String>::iterator j = m_advertised.begin(); j != m_advertised.end(); ++j) {
            ...
            if (WildcardMatch((*j), wkn)) {
                QCC_DbgPrintf(("SimpleNameService::HandleProtocolQuestion(): request for %s does not match my %s\n",
                               wkn.c_str(), (*j).c_str()));
                continue;
            } else {
                respond = true;
...
void SimpleNameService::HandleProtocolAnswer(IsAt isAt, uint32_t timer, qcc::IPAddress address)
{
    QCC_DbgPrintf(("SimpleNameService::HandleProtocolAnswer()\n"));
    ...
    if (m_callback == 0) {
        QCC_DbgPrintf(("SimpleNameService::HandleProtocolAnswer(): No callback, so nothing to do\n"));
        return;
    }
    ...
    for (uint8_t i = 0; i < isAt.GetNumberNames(); ++i) {
        QCC_DbgPrintf(("SimpleNameService::HandleProtocolAnswer(): Got well-known name %s\n", isAt.GetName(i).c_str()));
    ...
    QCC_DbgPrintf(("SimpleNameService::HandleProtocolAnswer(): Got QUID %s\n", guid.c_str()));
    ...
    QCC_DbgPrintf(("SimpleNameService::HandleProtocolAnswer(): Got IP %s from protocol\n", recvfromAddress.c_str()));

    if (isAt.GetIPv4Flag()) {
        ipv4address = isAt.GetIPv4();
        QCC_DbgPrintf(("SimpleNameService::HandleProtocolAnswer(): Got IPv4 %s from message\n", ipv4address.c_str()));
    }

    if (isAt.GetIPv6Flag()) {
        ipv6address = isAt.GetIPv6();
        QCC_DbgPrintf(("SimpleNameService::HandleProtocolAnswer(): Got IPv6 %s from message\n", ipv6address.c_str()));
    }

    uint16_t port = isAt.GetPort();
    QCC_DbgPrintf(("SimpleNameService::HandleProtocolAnswer(): Got port %d from message\n", port));
    ...
    QCC_DbgPrintf(("SimpleNameService::HandleProtocolAnswer(): Calling back with %s\n", addrbuf));
...
void SimpleNameService::HandleProtocolMessage(uint8_t const* buffer, uint32_t nbytes, qcc::IPAddress address)
{
    QCC_DbgPrintf(("SimpleNameService::HandleProtocolMessage(0x%x, %d, %s)\n", buffer, nbytes, address.ToString().c_str()));

#if HAPPY_WANDERER
    if (Wander() == false) {
        QCC_LogError(ER_FAIL, ("SimpleNameService::HandleProtocolMessage(): Wander(): out of range"));
        return;
    } else {
        QCC_LogError(ER_FAIL, ("SimpleNameService::HandleProtocolMessage(): Wander(): in range"));
    }
#endif
    ...
    if (bytesRead != nbytes) {
        QCC_DbgPrintf(("SimpleNameService::HandleProtocolMessage(): Deserialize(): Error\n"));
        return;
    }
