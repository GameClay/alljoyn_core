...
void AllJoynObj::BindSessionPort(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    if (status == ER_OK) {
        ...
        if (srcEp != NULL) {
            if (opts.transports & TRANSPORT_BLUETOOTH) {
                bool allowed = router.GetPermissionDB().IsBluetoothAllowed(*srcEp);
                if (!allowed) {
                    opts.transports ^= TRANSPORT_BLUETOOTH;
                    QCC_LogError(ER_ALLJOYN_PERMISSION_ERROR, ("AllJoynObj::BindSessionPort WARNING: No permission to use Bluetooth"));
                }
            }
            if (opts.transports & TRANSPORT_WLAN) {
                bool allowed = router.GetPermissionDB().IsWifiAllowed(*srcEp);
                if (!allowed) {
                    opts.transports ^= TRANSPORT_WLAN;
                    QCC_LogError(ER_ALLJOYN_PERMISSION_ERROR, ("AllJoynObj::BindSessionPort WARNING: No permission to use Wifi"));
                }
            }
...
ThreadReturn STDCALL AllJoynObj::JoinSessionThread::Run(void* arg)
{
    ...
    /* Get the sender */
    BusEndpoint* srcEp = ajObj.GetDaemonRouter().FindEndpoint(sender);
    if (srcEp != NULL) {
        if (optsIn.transports & TRANSPORT_BLUETOOTH) {
            bool allowed = ajObj.GetDaemonRouter().GetPermissionDB().IsBluetoothAllowed(*srcEp);
            if (!allowed) {
                optsIn.transports ^= TRANSPORT_BLUETOOTH;
                QCC_LogError(ER_ALLJOYN_PERMISSION_ERROR, ("AllJoynObj::JoinSessionThread WARNING: No permission to use Bluetooth"));
            }
        }
        if (optsIn.transports & TRANSPORT_WLAN) {
            bool allowed = ajObj.GetDaemonRouter().GetPermissionDB().IsWifiAllowed(*srcEp);
            if (!allowed) {
                optsIn.transports ^= TRANSPORT_WLAN;
                QCC_LogError(ER_ALLJOYN_PERMISSION_ERROR, ("AllJoynObj::JoinSessionThread WARNING: No permission to use Wifi"));
            }
        }

        if (optsIn.transports == 0) {
            status = ER_BUS_NO_TRANSPORTS;
        }
    }
    ...
    if (status != ER_OK) {
        ...
        if (rSessionEp) {
            ...
            if (joinerEp && foundSessionMapEntry) {
                ...
                if (replyCode == ALLJOYN_JOINSESSION_REPLY_SUCCESS) {
                    if (!isAccepted) {
                        ...
                        if (status != ER_OK) {
                            replyCode = ALLJOYN_JOINSESSION_REPLY_FAILED;
                            QCC_LogError(status, ("AddSessionRoute(%u, %s, NULL, %s, NULL) failed", newSessionId, sender.c_str(), rSessionEp->GetUniqueName().c_str()));
                        }
                        if (status == ER_OK) {
                            ajObj.AcquireLocks();
                            if (sme.opts.isMultipoint) {
                        ...
                        if (status == ER_OK) {
                            ...
                            if (it != ajObj.sessionMap.end()) {
                                it->second.fd = fds[0];
                                it->second.memberNames.push_back(sender);
            ...
            if (replyCode == ALLJOYN_JOINSESSION_REPLY_SUCCESS) {
                BusEndpoint* joinerEp = ajObj.router.FindEndpoint(sender.c_str());
                if (joinerEp) {
    ...
    if ((replyCode == ALLJOYN_JOINSESSION_REPLY_SUCCESS) && sme.opts.isMultipoint) {
        for (size_t i = 0; i < sme.memberNames.size(); ++i) {
            ...
            ajObj.AcquireLocks();
            BusEndpoint* joinerEp = ajObj.router.FindEndpoint(sender.c_str());
            BusEndpoint* memberEp = ajObj.router.FindEndpoint(member.c_str());
            ...
            if (memberEp && (memberEp->GetEndpointType() == BusEndpoint::ENDPOINT_TYPE_VIRTUAL)) {
                ...
                pair<String, SessionId> key(member, id);
                multimap<pair<String, SessionId>, SessionMapEntry>::iterator it = ajObj.sessionMap.find(key);
                if (it != ajObj.sessionMap.end()) {
                    it->second.memberNames.push_back(sender);
                }
        ...
        while (it != b2bEndpoints.end()) {
            ...
            if (sendSet.find(dest) == sendSet.end()) {
                ...
                if (status != ER_OK) {
                    ...
                    QCC_LogError(status, ("Error sending org.alljoyn.Daemon.DetachSession signal to %s", dest.c_str()));
                }
            }
            ++it;
        }
...
void AllJoynObj::AttachSession(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    if (status != ER_OK) {
        ...
        AcquireLocks();
        BusEndpoint* destEp = router.FindEndpoint(destStr);
        ...
        if (destEp && ((destEp->GetEndpointType() == BusEndpoint::ENDPOINT_TYPE_REMOTE) ||
                    ...
                    if (status == ER_OK) {
                        ...
                        if (isAccepted && creatorEp) {
                            ...
                            if (smIt != sessionMap.end()) {
                                smIt->second.memberNames.push_back(srcStr);
                                id = smIt->second.id;
                                destIsLocal = true;
                            ...
                            if (replyCode == ALLJOYN_JOINSESSION_REPLY_SUCCESS) {
                                if (optsOut.traffic == SessionOpts::TRAFFIC_MESSAGES) {
                                    RemoteEndpoint* tEp = NULL;
                                    status = router.AddSessionRoute(id, *destEp, tEp, *srcEp, srcB2BEp);
                                    if (ER_OK != status) {
                                        QCC_LogError(status, ("AddSessionRoute(%u, %s, NULL, %s) failed", id, dest, srcEp->GetUniqueName().c_str(), srcB2BEp ? srcB2BEp->GetUniqueName().c_str() : "NULL"));
                                    }
                                }
                            }
                        } else {
            ...
            if (b2bEpName.empty()) {
                ...
                if ((status == ER_OK) && (replyCode == ALLJOYN_JOINSESSION_REPLY_SUCCESS)) {
                    ...
                    while (replyCode == ALLJOYN_JOINSESSION_REPLY_SUCCESS) {
                        /* Does vSessionEp route through b2bEp? If so, we're done */
                        BusEndpoint* ep = router.FindEndpoint(destStr);
    ...
    if ((replyCode == ALLJOYN_JOINSESSION_REPLY_SUCCESS) && optsOut.isMultipoint && (id != 0) && destIsLocal) {
        SendMPSessionChanged(id, srcStr.c_str(), true, destStr.c_str());
    }
    ...
    if (srcEp != NULL) {
        if (transports & TRANSPORT_BLUETOOTH) {
            ...
            if (!allowed) {
                ...
                QCC_LogError(ER_ALLJOYN_PERMISSION_ERROR, ("AllJoynObj::AdvertiseName WARNING: No permission to use Bluetooth"));
            }
        }
        if (transports & TRANSPORT_WLAN) {
            ...
            if (!allowed) {
                ...
                QCC_LogError(ER_ALLJOYN_PERMISSION_ERROR, ("AllJoynObj::AdvertiseName WARNING: No permission to use Wifi"));
            }
        }
    }
            ...
            for (size_t i = 0; i < transList.GetNumTransports(); ++i) {
                ...
                if (trans && (srcEp != NULL)) {
                    if (trans->GetTransportMask() & TRANSPORT_BLUETOOTH && !router.GetPermissionDB().IsBluetoothAllowed(*srcEp)) {
                        QCC_LogError(ER_ALLJOYN_PERMISSION_ERROR, ("AllJoynObj::FindAdvertisedName WARNING: No permission to use Bluetooth"));
                        continue;
                    }
                    if (trans->GetTransportMask() & TRANSPORT_WLAN && !router.GetPermissionDB().IsWifiAllowed(*srcEp)) {
                        QCC_LogError(ER_ALLJOYN_PERMISSION_ERROR, ("AllJoynObj::FindAdvertisedName WARNING: No permission to use Wifi"));
                        continue;
                    }
